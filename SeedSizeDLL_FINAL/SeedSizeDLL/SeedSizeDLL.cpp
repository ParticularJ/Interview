...
int plantInfo::getImage(cv::Mat &src, cv::Mat &dst, cv::Mat &final_pic) {
...
std::pair<float, float> plantInfo::caculateAvg(std::priority_queue <std::pair<int, cv::RotatedRect>, std::vector<std::pair<int, cv::RotatedRect> >, cmpH> H) {
	...
	return std::make_pair(height / (size-3), width / (size-3));
}
...
void plantInfo::putROI(cv::Mat &roi, cv::Mat &final_pic, int flag) {
	cv::Mat roi_img;
