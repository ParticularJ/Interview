...
private:
	...
	pair<float, float> caculateAvg(priority_queue <pair<int, RotatedRect>, vector<pair<int, RotatedRect> >, cmpH> H) {
		...
		float width = 0;
		size_t size = H.size();
		int pop_size = contours.size() * 0.2;
	...
	//	cout << height<<" "<< size << " " << width << endl;
		return make_pair(height / (size- 2*pop_size), width / (size- 2*pop_size));
	}
	...
	void putROI(Mat &roi, int flag, int ID) {
		...
		if (local + roi.rows < final_pic.rows) {
			if (flag == 0) {
				...
				putText(final_pic, "ID:" + to_string(ID), Point(offset + roi.cols + 10, (local + roi.rows)), 2, 1, Scalar(255, 0, 0), 2);
			}
			else {
				roi_img = (final_pic(Range(local, local + roi.rows), Range(offset + static_cast<int>(avgHeight) * 3 + static_cast<int>(avgWidth), offset + static_cast<int>(avgHeight) * 3 + static_cast<int>(avgWidth) + roi.cols)));
				putText(final_pic, "ID:" + to_string(ID), Point(offset + static_cast<int>(avgHeight) * 3 + static_cast<int>(avgWidth) + roi.cols + 10, (local + roi.rows)), 2, 1, Scalar(255, 0, 0), 2);
			}
		}
		else {
			if (flag == 0) {
				roi_img = (final_pic(Range(new_line, roi.rows + new_line), Range(offset + 2*static_cast<int>(avgHeight), offset + 2*static_cast<int>(avgHeight) + roi.cols)));
				putText(final_pic, "ID:" + to_string(ID), Point(offset + 2 * static_cast<int>(avgHeight) + roi.cols + 10, (roi.rows + new_line)), 2, 1, Scalar(255, 0, 0), 2);
				new_line += (roi.rows +20);
			}
			else {
				roi_img = (final_pic(Range(new_line, roi.rows + new_line), Range(offset + 2*static_cast<int>(avgHeight) * 3 , offset + 2*static_cast<int>(avgHeight) * 3  + roi.cols)));
				putText(final_pic, "ID:" + to_string(ID), Point(offset + 2 * static_cast<int>(avgHeight) * 3 + roi.cols + 10, (roi.rows + new_line)), 2, 1, Scalar(255, 0, 0), 2);
				new_line += (roi.rows+20);
			}
		}
		roi.copyTo(roi_img);
	...
	void displayHeight(priority_queue <pair<int, RotatedRect>, vector<pair<int, RotatedRect> >, cmpH>& myqueH, Mat &dst) {
		...
		//	cout << "The area:" << res.size.area() << endl;
			...
			if (abs(height - avgHeight) > avgHeight/ 3 || abs(width - avgWidth) > avgWidth/ 3) {
				// 展示错误图像
				for (int j = 0; j <= 3; j++) {
					//	//	//cout << P[j];
...
//	src = imread("2.jpg");
	...
	// 测试图片集
	src = imread("D://Myself//Dataset//rice//E4.jpg", 0);
	cout << src.channels() << endl;
	dst = imread("D://Myself//Dataset//seed//D4.jpg");
