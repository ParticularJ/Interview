...
private:
	...
	pair<pair<float, float>, float> caculateAvg(priority_queue <pair<int, RotatedRect>, vector<pair<int, RotatedRect> >, cmpH> H) {
		...
		while (H.size()-pop_size) {
			...
			height += h;
			width += w;
			area += contourArea(contours[H.top().first]);
			H.pop();
		}
	
	//	cout << height<<" "<< size << " " << width << endl;
		return make_pair(make_pair(height / (size- 2*pop_size), width / (size- 2*pop_size)), area/(size-2*pop_size));
	}
	...
	void putLength(string name, int Length) {
		...
		// 书写总长度
		putText(final_pic, name + to_string(Length), Point(semit_cenX-150, y_end + 15), 2, 1, Scalar(0, 0, 255), 2);
...
//			}
	...
	//	putLength("Total Height: ", totalHeight);
		offset = dst.cols;
		//display(dst, "resH");
	}
...
//			}
	...
	//	putLength("Total Width: ", totalWidth);
		offset = 20;
	}
...
private:
	...
	void SortSeed(Mat &src, Mat &dst) {
		...
		findRect(dst);
		//cout << myqueH.size();
		pair<pair<float, float>, float> avg = caculateAvg(myqueH);
...
//	src = imread("2.jpg");
	...
	// 测试图片集
	src = imread("D://Myself//Dataset//rice//E10.jpg", 0);
	dst = imread("D://Myself//Dataset//seed//D10.jpg");
